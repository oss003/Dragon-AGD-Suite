/* Dragon AGD SCript Compiler  Version 1.7   2019-06-27      */
/* using AGD 6809 engine Version V1.4        2019-06-28		 */
/*                                                           */
...
int main( int argc, const char* argv[] )
{
	...
	if ( argc == 3 )
	{
		...
		nMessageNumber = 0;
		flgParm = atoi(argv[2]);
		if (flgParm == 1 || flgParm == 3 || flgParm == 5 || flgParm == 7)
		{
		   flagB = 1;
		}
		else
		...
		if (flgParm == 2 || flgParm == 3 || flgParm == 6 || flgParm == 7)
		{
		   flagR = 1;
		}
		else
...
void CreateMessages( void )
{
	...
	short int nStart = 1;									/* line start */
	unsigned short int invSlsh = 0;							/* inverted slash found */
	unsigned short int alreadyDone = 0;						/* quote and comma added */

	cSrc = cBufPos;											/* Set up source address. */

	/* reset compilation address. */
	nCurrent = nAddress;									/* pass actual pointer address to variable */
	nNextLabel = 0;											/* don't know what does this here */

	cObjt = cStart + ( nCurrent - nAddress );				/* equals cObjt = cStart  due to previous assignement */
	WriteText( "\nmsgDat" );								/* write section header */

	while ( ( cSrc - cBuff ) < lSize )	{
		alreadyDone = 0;									/* reset flags */
		invSlsh = 0;
		while ( *cSrc < 128 && *cSrc != 13)	{				/* not end of line or sentence marker */
			alreadyDone = 0;								/* reset flah for slash processed */
			if ( *cSrc == 39 ) {							/* if an apostroph is found */
				if ( nStart ) {								/* if at line start (should not happen) */
					WriteText( "\n        fcc " );			/* start of text message */
				}
				else {										/* if at end of line */
					WriteText( "\"" );						/* close quote */
					alreadyDone = 1;						/* done for case apostrophe */
				}
				WriteNumber( *cSrc++ );						/* write as numeric outside quotes */
				nStart = 1;									/* flag line start */
			}
			else	{										/* for normal chars */
				if ( nStart )	{							/* if line beginning */
					WriteText( "\n        fcc \"" );		/* start of text message */
					nStart = 0;								/* reset flag */
				}
				if ( *cSrc == 92 )	{						/* if backslash found   */
					*cObjt = *cSrc;							/* add one extra 		*/
					...
					invSlsh = 1;							/* set found flag */
				}
				*cObjt = *cSrc;								/* add backslash to output */
				cSrc++;										/* advance pointers */
				cObjt++;
				nCurrent++;									/* and counter */
			}
		}
		alreadyDone = 0;									/* reset flag */
		if ( nStart ) {										/* if line beginning */
			WriteText( "\n        fcc " );					/* start of text message */
		}
		else {												/* if end of line */
			WriteText( "\"" );								/* close quote */
			alreadyDone = 2;								/* done for case that have bit7 set */
		}
															/* Now going to put number (end of message code)	*/
		if (*cSrc == 141 || *cSrc == 13) {					/* if enter or enter with bit7 set */
			WriteText( ",");								/* add comma */
		}
		if (*cSrc == 141 || *cSrc == 13) {					/* if enter or enter with bit7 set */
			if ( alreadyDone == 0 || invSlsh == 1 )  {		/* if not yet added quote and comma OR backslah detected */
				if ( nStart == 0 )  {						/* if not at line begining */
					WriteText( ",");						/* add comma */
				}
			}
			alreadyDone = 0;								/* reset flags */
			invSlsh = 0;									/* clear flag */
		}
		WriteNumber( *cSrc++ );								/* write last char with terminator bit */
		nStart = 1;											/* set line beginning */
	}

	/* Number of messages */
	WriteText( "\nnumMsg  fcb " );							/* start of text message */
	WriteNumber( nMessageNumber );							/* write number of messages found */
}
...
//		WriteInstruction( "ld hl,fx1" );					/* sounds beginning */
...
void CR_Other( void )
{
	...
	WriteInstruction( "sty  <reg_IX	; testing" );	// update presudo register
}
...
void CR_DrawRoom( void )
{
	WriteInstruction( "jsr  DRoom	; DRAW SCREEN" );
}
...
void CR_Crumble( void )
{
	WriteInstruction( "jsr  Crumble" );					// call appropriate routine
	WriteText( "    ; CRUMBLING" );						// show command
}
